...
export default function ShopCar() {
    ...
    return (
        <Template>
            <ComponentSkeleton paddingTop={4} paddingBottom={2} lg={12} md={12} xs={12} gridTemplateColumns={'1fr 1fr 1fr'}>
                    ...
                    shopCar.length !== 0 ? <LoadingContext>
                            {shopCar.map(product => (
                                <Grid key={product.nombre}>
                                    <Card product={product} dispatcher={deleteDispatcher} children={<ShoppingCartCheckoutIcon/>}/>
                                </Grid>))}
                        </LoadingContext> :
                        <>
                            <CustomBoxOne_Template>
